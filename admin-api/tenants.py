...
exten => 1000,1,Answer()
 ...
 same => n,Set(OVERFLOW_TIMEOUT=${{IF($["${{DB(config-${{TENANT}}/overflow-timeout-segundos)}}" = ""]?${{OVERFLOW_TIMEOUT_SECONDS}}:${{DB(config-${{TENANT}}/overflow-timeout-segundos)}})}})
 same => n,Queue(${{FILA_IDIOMA}},c,,,${{OVERFLOW_TIMEOUT}})
